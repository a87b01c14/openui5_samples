sap.ui.define([
	...
	function (Controller, JSONModel, MessageBox, UI5Date) {
		...
		return Controller.extend("sap.m.sample.PlanningCalendarRelativeViews.Page", {
			onInit: function () {
				// create model
				var oModel = new JSONModel();
				oModel.setData({
					startDate: UI5Date.getInstance("2017", "0", "15", "8", "0"),
					minDate: UI5Date.getInstance("2017", "0", "15", "8", "0"),
					people: [{
						...
						{
							pic: "https://sdk.openui5.org/test-resources/sap/ui/documentation/sdk/images/Donna_Moore.jpg",
							name: "Donna Moore",
							role: "team member",
							appointments: [
								{
				...
				var oWeeksRelativeView = this.getView().byId("PC12").getViews().filter( function(oView) { return oView.getKey() === "A"; })[0];
				var oShiftsRelativeView = this.getView().byId("PC12").getViews().filter( function(oView) { return oView.getKey() === "D"; })[0];
				oWeeksRelativeView.setIntervalLabelFormatter(this.intervalLabelFormatterForWeeks);
				oShiftsRelativeView.setIntervalLabelFormatter(this.intervalLabelFormatterForShifts);
			},
			intervalLabelFormatterForWeeks: function(iIntervalIndex) {
				return "Week " + (iIntervalIndex + 1);
			},
			intervalLabelFormatterForShifts: function(iIntervalIndex) {
